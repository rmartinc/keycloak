...
public class TrustedDeviceCredentialProvider implements CredentialProvider<TrustedDeviceCredentialModel>, CredentialInputValidator {
    ...
    public void createTrustedDeviceCredential(RealmModel realm, UserModel user) {
        ...
        // Set cookie
        setTrustedDeviceCookie(user.getId(), credential, realm.getAttribute(
                TrustedDeviceAuthenticatorFactory.REALM_TRUST_DEVICES_EXPIRATION_ATTR, CookieMaxAge.WEEK));
    ...
    public void deleteExpiredCredentials(RealmModel realm, SubjectCredentialManager credentialManager) {
        long expirationLimit = Time.currentTimeMillis() - 604800;
    ...
    public boolean isValid(RealmModel realm, UserModel user, CredentialInput credentialInput) {
        ...
        long credentialExpiresAt = tdCredential.getCreatedDate() + realm.getAttribute(
                TrustedDeviceAuthenticatorFactory.REALM_TRUST_DEVICES_EXPIRATION_ATTR, CookieMaxAge.WEEK);
    ...
    private String generateSecret() {
        byte[] bytes = SecretGenerator.getInstance().randomBytes();
