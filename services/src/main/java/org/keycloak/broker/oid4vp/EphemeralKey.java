...
public record EphemeralKey(String kid, JWK publicJwk, String privateKeyPkcs8Base64) {
    ...
    public static EphemeralKey generate() {
        ...
        publicJwk.setKeyId(thumbprint(publicJwk));
        String encoded = Base64.getEncoder().encodeToString(keyPair.getPrivate().getEncoded());
        return new EphemeralKey(publicJwk.getKeyId(), publicJwk, encoded);
    }

    // The RFC 7638 thumbprint, computed on the parsed form because the thumbprint util reads the key
    // members from the raw claims.
    private static String thumbprint(JWK publicJwk) {
        try {
            JWK normalized = JsonSerialization.readValue(
                    JsonSerialization.writeValueAsString(publicJwk), JWK.class);
            String thumbprint = JWKSUtils.computeThumbprint(normalized);
            if (thumbprint == null) {
                throw new IllegalStateException("Failed to compute the ephemeral key thumbprint");
            }
            return thumbprint;
        } catch (IOException e) {
            throw new IllegalStateException("Failed to compute the ephemeral key thumbprint", e);
        }
    }
