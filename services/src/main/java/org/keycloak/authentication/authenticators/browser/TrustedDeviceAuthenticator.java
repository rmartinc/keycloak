...
public class TrustedDeviceAuthenticator extends AbstractFormAuthenticator implements Authenticator, CredentialValidator<TrustedDeviceCredentialProvider> {
    ...
    public boolean configuredFor(KeycloakSession session, RealmModel realm, UserModel user) {
        ...
        if (trustDeviceNote == null) {
            // set note to show the teust device checkbox in 2FA
            authSession.setAuthNote(TrustedDeviceAuthenticatorFactory.TRUST_DEVICE_AUTH_NOTE, TrustedDeviceAuthenticatorFactory.TRUST_DEVICE_SHOW);
        }
